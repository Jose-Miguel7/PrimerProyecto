new Vue({
    ...
    data: {
        ...
        pais: '',
        colores: ['#0779e4', '#d63447', '#787878', '#54e346', '#ffd66b']
    },
    watch: {
    ...
    methods: {
        ...
        obtenerDatos: function() {
            ...
            axios.get('/api/semanal?pais=' + this.pais)
                .then((response) => {
                    ...
                    for (let i = 0; i < paises.length; i++) {
                        var dataSets = {
                            label: 'Ventas Semanales ' + paises[i],
...
let chartSemanal = (fechas, datos) => {
    document.getElementById('chart-semanal').remove();
    let canvas = document.createElement('canvas')
    canvas.id = 'chart-semanal';
    document.querySelector('.chart-container').appendChild(canvas)
    var ctxMensual = document.getElementById('chart-semanal').getContext('2d');
